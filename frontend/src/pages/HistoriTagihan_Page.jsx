...
const HistoriTagihan = () => {
  ...
  const [searchParam] = useState(["tanggalLayanan"]);

  // Dummy data
  const datas = Array.from({ length: 30 }, (_, index) => ({
    tagihan: `172746462${index + 1}`,
    layanan: `Bronze`,
    price: `16.500`,
    status: "PENDING",
    pembayaran: "Transfer-BNI",
    tanggalTagihan: `${index + 1} Sep 2024`,
    tanggalLayanan: `${index + 2} Sep 2024`,
  }));
            ...
            {currentRows.map((row, index) => (
              <tr key={index} className="bg-white hover:bg-gray-50">
                <td className="px-6 py-4">
                  {row.layanan} <br />
                  <span className="text-orange-400">{row.tanggalLayanan}</span>
                </td>
        ...
        {currentRows.map((row, index) => (
          ...
          >
            <div className="flex justify-between">
              <div>
                <p className="font-bold text-cyan-600">{row.tagihan}</p>
                <p className="text-sm text-orange-400">{row.tanggalTagihan}</p>
              </div>
            ...
            <div className="mt-2">
              ...
              <p className="text-sm mt-1">
                <span className="font-medium">PAYMENT:</span> {row.pembayaran}
              </p>
            </div>
